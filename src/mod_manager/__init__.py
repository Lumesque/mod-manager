...
@click.option('-i', '--ignore-dependencies', multiple=True, default=[], help="Manually ignore package dependencies by name")
...
@click.option('-s', '--no-save', is_flag=True, default=False, help="Don't save a 'versions.json' file for future use with 'redownload'")
@click.option('-o', '--output-directory', type=click.Path(exists=True, path_type=Path, dir_okay=True), help="Directory to write the zip files to", default=Path('.'))
@click.pass_context
...
def download(ctx):
    ...
    version_list = downloader.get_download_list_by_name(ctx.obj['MODS'], ignore_dependencies=ctx.obj['IGNORE_DEPENDENCIES'])
